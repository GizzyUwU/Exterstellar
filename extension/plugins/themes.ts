...
Exterstellar.register({
  ...
  start() {
    ...
    if (!base) {
      console.warn(
        `[Exterstellar | themes] Unknown flavor "${flavor}" for theme "${theme}"`,
      );
      return;
    }
    ...
    sessionStorage.setItem(
      "_ext_themes_pre",
      `${theme}:${flavor}:${accent}:${accentAll ? "1" : "0"}`,
    );
  ...
  if (_t && _f) {
    ...
    if (_base) {
      ...
      if (_accentHex) {
        ...
        _vars["--color-space-accent-soft"] =
          `rgb(${hexToRgb(_accentHex)} / 0.18)`;
